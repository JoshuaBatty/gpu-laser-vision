...
mod kernels {
    ...
    pub fn edge_detect(
        ...
        if let Some((edge, idx)) = b.get_mut_indexed() {
            let i = idx.get();
            let x = i % w;
            let y = i / w;
            ...
            // Store a normalized edge strength.
            *edge = (gx.abs() + gy.abs()).min(1.0);
        }
    }
}
...
fn main() {
    ...
    let grayscale: Vec<u8> = normalized_f32_to_u8(&grayscale_host);
    let edges: Vec<u8> = normalized_f32_to_u8(&edges_host);
    ...
    let edges_img = image::GrayImage::from_raw(rgba.width(), rgba.height(), edges)
        .expect("edge buffer has the wrong length");
...
fn normalized_f32_to_u8(values: &[f32]) -> Vec<u8> {
    values
        .iter()
