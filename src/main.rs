...
fn main() -> Result<()> {
    ...
    let img = ImageReader::open("assets/test_tiles.png")
        .context("opening assets/test_tiles.png")?
        .decode()?;
    ...
    // Save a grayscale buffer using the source image dimensions.
    let save_image = |path: &str, data: Vec<u8>| {
        image::save_buffer(
            path,
            &data,
            rgba.width(),
    ...
    save_image("assets/circle-grayscale.png", grayscale)?;
    save_image("assets/circle-edges.png", edges)?;
    save_image("assets/circle-thin-edges.png", thin_edges)?;
    save_image("assets/circle-edge-classes.png", edge_classes)?;
    save_image("assets/circle-connected-edges.png", connected_edges)?;
