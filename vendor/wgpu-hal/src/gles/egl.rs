    ...
    for tier_max in (0..tiers.len()).rev() {
        ...
        match egl.choose_first_config(display, &attributes) {
            Ok(Some(config)) => {
                ...
                let allow_non_native_presentation =
                    std::env::var_os("WGPU_EGL_ALLOW_NON_NATIVE_PRESENTATION").is_some();
                let tier_threshold = if allow_non_native_presentation
...
impl crate::Instance for Instance {
    ...
    unsafe fn init(desc: &crate::InstanceDescriptor<'_>) -> Result<Self, crate::InstanceError> {
        ...
        let (display, wsi_kind) = match (desc.display.map(|d| d.as_raw()), egl1_5) {
            ...
            (None, Some(egl))
                if std::env::var_os("WGPU_EGL_FORCE_X11").is_some()
                    && client_ext_str.contains("EGL_EXT_platform_x11") =>
            {
                log::debug!("Using the default X11 platform");
                let display = unsafe {
                    egl.get_platform_display(
                        EGL_PLATFORM_X11_KHR,
                ...
                .map_err(instance_err("failed to get default X11 display"))?;
                (display, WindowKind::X11)
            }
